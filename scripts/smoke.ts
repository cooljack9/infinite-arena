...
import { TRAIT_CFG, ALL_TRAIT_IDS } from '../packages/core/src/content/traits';
...
// ── 3. 特性实际生效 ──
console.log('\n[3] 特性运行时可观测');
// vX：模板不再绑定固定特性，特性在 variateHero 时从全池随机分配（用户需求「所有角色都随机产生」）。
// 验证：① 每个英雄经 variateHero 都拿到池中有效特性；② 跨种子覆盖多种特性（非全员同特性）；
//       ③ 同 seed 必得同特性（不破坏回放）；④ 特性在派生数值上真实生效（确定性单元校验）。
...
ok(
  ...
  'variateHero 给每个英雄分配了池中有效特性',
);
const distinctTraits = new Set(variated.map((h) => h.traitId));
ok(distinctTraits.size >= 2, `随机分配覆盖多种特性（${distinctTraits.size} 种 / 9 英雄）`);
ok(
  variateHero(HEROES[0], 777).traitId === variateHero(HEROES[0], 777).traitId,
  'variateHero 同 seed 特性一致（可复现）',
);

// 特性在派生数值上真实生效（确定性单元校验，比"日志里有没有关键词"更硬）
const base = makeAlly({ ...HEROES[0] }, 1, []);
const sl = makeAlly({ ...HEROES[0], traitId: 'slowburn' }, 1, []);
ok(Math.abs(sl.derived.atkSpeed / base.derived.atkSpeed - 0.7) < 1e-6, 'slowburn 开局攻速 ×0.7');
ok(Math.abs(sl.derived.pDmg / base.derived.pDmg - 0.6) < 1e-6, 'slowburn 开局双攻 ×0.6');
ok(Math.abs(sl.derived.mDmg / base.derived.mDmg - 0.6) < 1e-6, 'slowburn 开局双攻 ×0.6');
const gr = makeAlly({ ...HEROES[0], traitId: 'grower' }, 1, []);
ok(Math.abs(gr.derived.pDmg / base.derived.pDmg - 0.85) < 1e-6, 'grower 全属性 ×0.85');
ok(Math.abs(gr.derived.hp / base.derived.hp - 0.85) < 1e-6, 'grower 生命 ×0.85');
ok(Math.abs((gr.sizeScale ?? 1) - (1 - TRAIT_CFG.growerBodyPct)) < 1e-9, 'grower 体型 -30%');
const rt = makeAlly({ ...HEROES[0], traitId: 'returner' }, 1, []);
ok(rt.derived.pResist === 0 && rt.derived.mResist === 0, '归来者防御恒为 0');
console.log('    随机特性抽样:', variated.map((h) => h.traitId).join(', '));
...
// vX：钉死三人原特性（momentum / volley / grace），还原该探针的标定基线——
// 随机化后模板不再绑特性，若放任随机特性，队伍 DPS 与奶量会漂移，占比阈值失去参照。
