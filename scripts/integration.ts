...
import { ALL_TRAIT_IDS } from '../packages/core/src/content/traits';
...
console.log('\n[6] 需求 3：角色面板算的是真实战斗属性（含装备预览差值）');
{
  ...
  const hero = HEROES[4];
  // vX：特性在 variateHero 时随机分配，故先个体化再 makeAlly（面板读到的就是这份副本的特性）
  const vh = variateHero(hero, 2024);
  const eq: Equipment[] = [generateEquipment(rng, 'red'), generateEquipment(rng, 'orange')];
  const base = makeAlly(vh, 8, []);
  const withEq = makeAlly(vh, 8, eq);
  const preview = makeAlly(vh, 8, [...eq, generateEquipment(rng, 'red')]);
...
console.log('\n[7] 需求 1：9 职业特性 / 技能二段 / 体型 数据完整');
{
  ...
  // 验证：每个职业经 variateHero 都拿到池中有效特性，且跨种子覆盖多种特性。
  const variated = HEROES.map((h, i) => variateHero(h, (20260808 ^ (i * 0x9e3779b1)) >>> 0));
  ok(
    variated.every((h) => h.traitId != null && ALL_TRAIT_IDS.includes(h.traitId)),
    '每个职业都经 variateHero 分配到池中特性',
  );
  const uniqTrait = new Set(variated.map((h) => h.traitId));
  ok(uniqTrait.size >= 2, `随机分配覆盖多种特性（${uniqTrait.size}）`);
...
console.log('\n[12] v2.1 需求：角色特性分离（同角色基础属性与体型个体差异化）');
{
  ...
  useGame.setState({ gold: 99999 });
  const id0 = g().run!.team[0].id;
  // v1.7：招募池在生成时即个体化，recruit() 直接采用池中英雄（不再二次个体化）。
  // 故「同名多份非克隆」由池个体化保证——注入两份不同种子个体化的同名英雄验证之。
  const poolBase = g().recruitPool;
  useGame.setState({ recruitPool: [...poolBase, variateHero(HEROES[0], 111)] });
  g().recruit(id0);
  useGame.setState({ recruitPool: [...g().recruitPool, variateHero(HEROES[0], 222)] });
  g().recruit(id0);
...
{
  // capFor 返回正确封顶
  ok(capFor('novice') === NOVICE_CAP && NOVICE_CAP === 10, `新手模式封顶 = NOVICE_CAP(${NOVICE_CAP})`);
  ...
  ok(g().endlessUnlocked === true, '新手模式通关封顶层（NOVICE_CAP） → 同时解锁普通无尽 + 铁人无尽');
...
{
  // ── 三模式封顶一致 ──
  ok(capFor('novice') === 10 && capFor('normal') === 500 && capFor('ironman') === 500,
     '三模式封顶：新手=10，普通/铁人均=500');
  ...
  ok(TUTORIAL.length === 10, `教学覆盖 10 组（实际 ${TUTORIAL.length}）`);
  const gotLayers = TUTORIAL.map((grp) => grp.layer);
  ok(JSON.stringify(gotLayers) === JSON.stringify([1, 1, 2, 3, 4, 5, 6, 7, 8, 10]), `教学层序 = ${gotLayers.join(',')}`);
  // v2.6：装备教学扩到三条链路（合成/附魔/重铸），组内步数不再恒为 2，只约束 2~4 步
  // ——超过 4 步一口气弹完会把玩家从操作里赶出去，那是教学在妨碍教学。
  // v1.8：移除「跳过已通关层」教学点后，layer-5 pre 组仅剩 1 步（冲刺通关），下限放宽到 1。
  ok(TUTORIAL.every((grp) => grp.steps.length >= 1 && grp.steps.length <= 4), '每组 1~4 个教学点（v1.8 起允许单步组）');
  ...
  console.log('\n[19] v2.9.4：随机奇遇（确定性 / 单次结算）+ 精英 Boss 标记 + 三子页教学锚点');
  {
    ...
    const HUB_ANCHORS = ['tut-guide', 'tut-hero-panel', 'tut-hero-sell', 'tut-skip5', 'tut-climb', 'tut-next-layer'];
    const TAB_ANCHORS = [
    ...
    const PRE_ANCHORS = ['tut-formation', 'tut-prebattle-start', 'tut-prebattle-talent', 'tut-prebattle-event', 'tut-prebattle-relic', 'tut-prebattle-vacuum', 'tut-prebattle-mutation']; // v1.8：移除 tut-prebattle-skip（跳过功能下线）；v1.9：UX-5 机制说明云锚点
