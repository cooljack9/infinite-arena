...
// ── [2] 找有奇遇的层（skipLayer 推进，最多探 12 层）──
...
// 推进到奇遇层
while ((useGame.getState().run?.layer ?? 0) < eventLayer) {
  const r = await getBackend().skipLayer({ runId: useGame.getState().run!.runId, idempotencyKey: `evt-sk-${Math.random().toString(36).slice(2, 8)}`, coreVersion: CORE_VERSION, bestLayer: 999 });
  if (r.ok) applySnapshot(useGame.setState, r.data); else { check('[2b] skipLayer', false, r.code); break; }
}
