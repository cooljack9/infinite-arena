...
// 推进一层（A5）
const skA = await getBackend().advanceLayerTo({ runId: useGame.getState().run!.runId, idempotencyKey: "netA-sk", coreVersion: CORE_VERSION, layer: useGame.getState().run!.layer + 1 });
check("A5 advanceLayerTo 成功", !!skA.ok, skA.ok ? "layer=" + skA.data.layer : skA.code);
net(null);
console.log("");
