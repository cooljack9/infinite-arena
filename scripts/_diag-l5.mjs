...
const r = await call("startRun", { heroIds: ["h_physTank", "h_charge", "h_healer"], mode: "normal", idempotencyKey: "diag-5-" + Math.random().toString(36).slice(2, 8), coreVersion: "4.1.0-detmath" });
const runId = r.data.runId;
for (let l = 1; l < 5; l++) {
  const sk = await call("skipLayer", { runId, idempotencyKey: "sk-" + l + "-" + Math.random().toString(36).slice(2, 8), coreVersion: "4.1.0-detmath", bestLayer: 999 });
  if (!sk.ok) { console.log("skip", l, "FAIL", sk.code); break; }
}
const b = await call("startBattle", { runId, idempotencyKey: "b-5", coreVersion: "4.1.0-detmath", formation: {}, clientTs: 0 });
