...
function handleStartBattle(p) {
  ...
  const battle = core.runBattle(row.snapshot, row.secret, p.formation ?? {});
  if (!battle.ok) return battle;
  const next = core.applySettlement(row.snapshot, row.secret, battle.data);
...
const handlers = {
  ...
  abandonRun:       (p) => {
    ...
    row.snapshot.status = 'lost';
    return ok(META);
  },
  skipLayer:        (p) => mutate(p, (s) => core.skipLayer(s)),
