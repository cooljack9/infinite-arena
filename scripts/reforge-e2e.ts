...
// ── [4] 跨层重置：advanceLayerTo 推进层后 reforgedThisLayer 回 false ──
const run = useGame.getState().run!;
const adv = await getBackend().advanceLayerTo({ runId: run.runId, idempotencyKey: 'ref-adv-1', coreVersion: CORE_VERSION, layer: run.layer + 1 });
if (adv.ok) {
  ...
  check('[4] advanceLayerTo', false, adv.code + '/' + adv.message);
}
