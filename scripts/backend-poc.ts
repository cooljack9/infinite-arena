...
async function main() {
  ...
  // ══ [6] 通用数据回路：后端权威查表（与前端同一份 DB）═══════
  console.log('\n[6] 通用数据回路：后端权威查表（DB 统一索引 + LRU 缓存）');
  const heroes = be.listContent('heroes');
  ok(Array.isArray(heroes.data) && heroes.data.length > 0, `列出英雄表 ${heroes.data.length} 条`);
  const oneHero = be.getContent('heroes', 'h_physTank');
  ok(oneHero.ok && oneHero.data?.id === 'h_physTank', '按主键取英雄 O(1)（与前端同一回路）');
  const rages = be.queryContent('relics', { id: { $in: ['r_rage'] } } as Record<string, unknown>);
  ok(rages.ok && Array.isArray(rages.data) && rages.data.length === 1,
    `带 $in 算子查询遗物表（命中 ${rages.ok ? rages.data.length : '?'} 条）`);
