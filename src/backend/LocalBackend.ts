...
export class LocalBackend implements GameBackend {
  ...
  async startBattle(req: StartBattleReq): Promise<Result<BattleResultDTO>> {
    return Promise.resolve(this.idem(req.idempotencyKey, () => {
      ...
      // preview：纯预热，只算回放、不结算、不落库、不推进 run（与 Edge 行为对齐）
      if (req.preview === true) {
        const sim = rules.runBattle(cur.snapshot, cur.secret, req.formation, req.battleOpts);
        if (!sim.ok) return sim as Result<BattleResultDTO>;
        const r = sim.data;
        return ok<BattleResultDTO>({
          ...
          replay: {
            ...
            layer: req.battleOpts?.effLayer ?? cur.snapshot.layer,
