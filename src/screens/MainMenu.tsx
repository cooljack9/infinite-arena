  ...
  return (
    <div className="app">
      <div className="panel accent col center">
        ...
        {/* vX 战场缩放（camera zoom）：1.0 全战场 / 1.25 温和拉近（默认）/ 1.5 明显拉近。账号级持久化。
            仅影响渲染合成期「整张战场围绕中心放大裁切」，零 sim/parity 影响；改完立刻生效。 */}
        <div className="col" style={{ marginTop: 12, gap: 4, width: '100%' }}>
          <div className="subtitle" style={{ margin: 0 }}>战场缩放</div>
          <div className="seg" role="radiogroup" aria-label="战场缩放">
            {([['auto', '自适应'], ['1', '全战场'], ['1.25', '拉近'], ['1.5', '特写']] as const).map(([v, label]) => {
              const isAuto = v === 'auto';
              const checked = isAuto
                ? cameraZoom === 'auto'
                : typeof cameraZoom === 'number' && Math.abs(cameraZoom - parseFloat(v)) < 0.001;
              return (
                <button
                  key={v}
                  type="button"
                  ...
                  className="seg-btn"
                  onClick={() => setCameraZoom(isAuto ? 'auto' : parseFloat(v))}
                >
                  {label}
                </button>
              );
          ...
          <div className="muted" style={{ fontSize: 10 }}>
            自适应：按屏幕宽度/分辨率自动缩放（窄屏看全场、宽屏/高分屏拉近更清晰）；手动档位会裁切战场边缘，自动战斗可能漏看边角单位
          </div>
        </div>

        {/* vX 镜头模式：全图 / 跟随队长。账号级持久化，仅影响渲染合成期，零 sim/parity 影响。 */}
        <div className="col" style={{ marginTop: 12, gap: 4, width: '100%' }}>
          <div className="subtitle" style={{ margin: 0 }}>镜头模式</div>
          <div className="seg" role="radiogroup" aria-label="镜头模式">
            {([['full', '全图'], ['follow', '跟随队长']] as const).map(([v, label]) => (
              <button
                key={v}
                type="button"
                role="radio"
                aria-checked={cameraMode === v}
                className="seg-btn"
          ...
          <div className="muted" style={{ fontSize: 10 }}>
            全图：始终看整张战场；跟随队长：镜头居中于所选队长（铁人模式队长阵亡后需重选）
          </div>
        </div>
