...
// MVP（友方 造成伤害+治疗+承伤加权 最高者）胜方额外获得 +1 随机一级属性成长奖励。
// 「继续」按钮按胜负与封顶层决定去路：胜→休整 / 封顶通关；负→重试战前 / 失败结算。
//
// v1.7 曾精简为「战后大家都获得了成长」一句话 + 可展开详情；
// v1.8c 恢复完整战报（用户要求），并把「击杀成长（累计）」挪到二次窗口（弹窗）查看。
//
...
const STAT_KEYS: (keyof PrimaryAttrs)[] = ['con', 'str', 'agi', 'int'];

type EvalRow = {
  id: string; side: 'ally' | 'enemy'; name: string; heroUid?: string;
  dmgDealt: number; dmgTaken: number; healDone: number; moveDist: number;
  isDead?: boolean;
};
...
export default function BattleEval() {
  ...
  // v3.4j 敌方全员：直接取 stats 里 side=enemy 的行（本地/云端都保证存在，sim.getBattleStats
  // 含全部非建筑单位；云端 outcome.stats 同样含敌方）。battleRemote.replay.enemies 仅作兜底
  // （本地模式 battleRemote 为 null，旧实现因此敌方空白）。
  let enemyRows = rows.filter((r) => r.side === 'enemy');
  if (!enemyRows.length) {
    const fallback = (battleRemote?.replay?.enemies ?? []).filter((u: { isBuilding?: boolean }) => !u.isBuilding);
    enemyRows = fallback.map((u: { id: string; name: string }) => ({
      id: u.id, side: 'enemy' as const, name: u.name,
      dmgDealt: 0, dmgTaken: 0, healDone: 0, moveDist: 0,
      isDead: false,
    }));
  }
  const enemies: EvalRow[] = enemyRows.map((r) => ({
    id: r.id, side: 'enemy' as const, name: r.name,
    dmgDealt: r.dmgDealt, dmgTaken: r.dmgTaken,
    healDone: r.healDone, moveDist: r.moveDist,
  })).sort((a, b) => (b.dmgDealt + b.healDone) - (a.dmgDealt + a.healDone));

  // MVP 姓名：我方列表里查（保证能显示名字，即便 stats 缺该 uid）
  const mvpName = mvpUid ? allies.find((r) => r.heroUid === mvpUid)?.name : undefined;

  // v1.8c 击杀成长（累计）——二次窗口内容：各勇者 growthBonus 永久累积
  const growthRows = run.team.map((h) => {
    const prim = h.growthBonus?.primary ?? {};
    const parts = STAT_KEYS.filter((k) => prim[k]).map((k) => `${STAT_CN[k]}+${prim[k]}`);
    return { uid: h.uid, name: h.name, text: parts.length ? parts.join('  ') : '暂无累计成长' };
  });
  ...
  const renderRow = (r: EvalRow) => {
    ...
    const isDead = !!r.isDead;
    return (
      <div
        key={r.id}
        className={'eval-row' + (isMvp ? ' mvp' : '')}
        style={isDead ? { opacity: 0.45, filter: 'grayscale(1)', textDecoration: 'line-through' } : undefined}
      >
        <span className="eval-name">
          {isMvp && <span className="eval-crown" title="本场 MVP">👑</span>}
          {isDead && <span title="本场阵亡">💀</span>}
          {r.name}
        </span>
  ...
  return (
    <div className="app">
      <div className="panel col eval-panel">
        <div className="row between" style={{ alignItems: 'center' }}>
          ...
          <div className="row" style={{ gap: 8 }}>
            ...
            <button
              className="chip"
              style={{ cursor: 'pointer', borderColor: '#6a4ad0', color: '#c9b6ff', background: 'transparent' }}
      ...
      {/* v1.8c 二次窗口：击杀成长（累计）——战斗击杀与成长药剂的永久属性累积 */}
      {growthOpen && (
        <div
          style={{
        ...
        >
          <div
            className="panel col"
            style={{ width: 400, maxWidth: '92vw', padding: 16 }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="title" style={{ fontSize: 16 }}>📊 击杀成长（累计）</div>
            <div className="muted" style={{ fontSize: 12, marginBottom: 10 }}>
              战斗击杀与成长药剂带来的<b style={{ color: '#ffd76a' }}>永久属性累积</b>，不随装备变化，越打越强。
            </div>
              ...
              {growthRows.map((r) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '7px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: 6,
                  }}
                >
                  <span style={{ fontWeight: 600, fontSize: 13 }}>{r.name}</span>
                  <span style={{ fontSize: 12, color: '#cfd6e4', textAlign: 'right' }}>{r.text}</span>
                </div>
              ))}
            </div>
            <div className="row" style={{ marginTop: 12, justifyContent: 'flex-end' }}>
              <button className="primary" onClick={() => setGrowthOpen(false)}>关闭</button>
            </div>
          </div>
        </div>
      )}
