  ...
  return (
    ...
    >
      <div
        ...
        style={{
          maxWidth: 560, width: '100%', maxHeight: '86vh', overflowY: 'auto',
          border: `1px solid ${info.color}`, borderRadius: 12, padding: 14,
          background: 'linear-gradient(180deg, rgba(22,28,44,0.98), rgba(12,16,26,0.98))',
      ...
      >
        ...
        <div className="row" style={{ gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={() => {
              ...
              setFxBusy('正在升星…');                   // 悬浮提示 + 锁全局交互
              upgradeHero(live.uid);
              setTimeout(() => { setUpgrading(false); setFxBusy(null); }, 3000);
            }}
          >
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          ...
          <div style={box}>
            ...
            <Stat label="暴击率" value={`${c.crit.toFixed(1)}%`} hint={withPreview ? delta(d.crit, c.crit) : undefined} />
            <Stat label="暴击伤害" value={`${c.critDmg.toFixed(0)}%`} hint={withPreview ? delta(d.critDmg, c.critDmg) : undefined} />
            <Stat label="物理减伤" value={`${c.pResist.toFixed(1)}%`} hint={withPreview ? delta(d.pResist, c.pResist) : undefined} />
            <Stat label="魔法减伤" value={`${c.mResist.toFixed(1)}%`} hint={withPreview ? delta(d.mResist, c.mResist) : undefined} />
            <Stat label="闪避" value={`${c.dodge.toFixed(1)}%`} hint={withPreview ? delta(d.dodge, c.dodge) : undefined} />
