...
import { archetypeOf, ARCHETYPE_INFO, personaVoice, lateBloomerScale } from '../game/persona';
...
export default function HeroPanel({ hero, level, equipment = [], preview, onClose }: Props) {
  ...
  // vX 成长原型（风格化母题）+ 晚成型无界膨胀判定
  const arch = archetypeOf(live);
  const archInfo = ARCHETYPE_INFO[arch];
  const isBloomer = arch === 'lateBloomer';
  ...
  // 人格化传记：意志履历 + 并肩 + 幸存层（P0）。从全局 growthLog / run 派生，跨刷新保留。
  const growthLog = useGame((s) => s.growthLog);
  const runState = useGame((s) => s.run);
  const bioMine = growthLog.filter((g) => g.uid === live.uid);
  const bioTags: Record<string, number> = {};
  for (const g of bioMine) for (const t of g.tags) bioTags[t] = (bioTags[t] ?? 0) + 1;
  const bioMates = (run?.team ?? []).filter((t) => t.uid !== live.uid).map((t) => displayName(t));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span
            style={{
              ...
              fontWeight: 800,
              fontSize: 17,
              transform: isBloomer ? `scale(${lateBloomerScale(level)})` : undefined,
              transformOrigin: 'left center',
              transition: 'transform .2s ease-out',
              textShadow: isBloomer ? `0 0 ${Math.round(6 + level * 0.3)}px ${archInfo.glow}` : undefined,
            }}
          >{displayName(live)}</span>
          ...
          {/* vX 成长原型：风格化母题徽章（神化 / 极致 / 无界膨胀） */}
          <span
            className="tag"
            style={{ color: archInfo.color, borderColor: `${archInfo.color}66`, boxShadow: `0 0 8px ${archInfo.glow}` }}
            title={`成长原型 · ${archInfo.motif}（${archInfo.tone}）`}
          >
            {archInfo.cn}
          </span>
        ...
        {/* 风格化声纹：原型母题 + 原型×个体 的台词，给角色「声纹」（纯展示，确定性） */}
        <div style={{ marginTop: 6, fontSize: 12, color: archInfo.color, fontStyle: 'italic', display: 'flex', gap: 6, alignItems: 'baseline' }}>
          <span>「{personaVoice(live)}」</span>
          <span className="muted" style={{ fontStyle: 'normal' }}>— {archInfo.motif}</span>
        </div>

        {/* 传记：人格化 P0 —— 意志履历 + 并肩 + 幸存层（让角色从属性载体变回「有经历的人」） */}
        <div style={{ marginTop: 8, border: `1px solid ${archInfo.color}33`, borderRadius: 8, padding: 8, background: 'rgba(255,255,255,0.025)' }}>
          <div className="subtitle" style={{ textAlign: 'left', marginBottom: 4, color: archInfo.color }}>📖 传记</div>
          <div style={{ fontSize: 12, color: '#ddd', lineHeight: 1.7 }}>
            <div>· 当前深度：第 <b>{runState?.layer ?? level}</b> 层（{archInfo.cn}之路）</div>
            {bioMates.length > 0 && <div>· 并肩：{bioMates.join('、')}</div>}
            {bioMine.length > 0 ? (
              <div>· 意志：共 {bioMine.length} 次抉择 〔{Object.entries(bioTags).map(([t, n]) => `${t}${n}`).join(' ')}〕</div>
            ) : (
              <div className="muted">· 意志：尚未做出成长抉择（每 2 层会主动向你请命）</div>
            )}
            {bioMine.length > 0 && (
              <div style={{ marginTop: 2, color: '#bbb' }}>
                最近：{bioMine.slice(-3).map((g) => `「${g.label}」`).join(' → ')}
              </div>
            )}
          </div>
        </div>
