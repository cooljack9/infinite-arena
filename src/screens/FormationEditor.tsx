...
import {
  FORMATION_PRESETS, FormationPreset, cellKey, deployableFor, presetFormation, toPos,
...
export default function FormationEditor({
  ...
  const clickCell = (c: number, r: number) => {
    ...
    if (!sel) return;
    const selHeroForCell = sel ? team.find((h) => h.uid === sel) : undefined;
    if (!deployableFor(selHeroForCell, arena, c, r)) return;
  ...
  return (
    <div className="col" style={{ gap: 8 }}>
      <div className="row between">
        <span className="tag">
          {sel
            ? `已选中「${selHero ? displayName(selHero) : ''}」→ ${
              selHero?.traitId === 'spacetime'
                ? '时空拓印：可站全图任意格，点格移动，或点另一名队员交换'
                : '点绿色格子移动，或点另一名队员交换'}`
            : '点击队员选中，再点绿色区域调整站位'}
        </span>
      ...
      >
        {Array.from({ length: rows }).flatMap((_, r) =>
          Array.from({ length: cols }).map((__, c) => {
            ...
            const deployable = deployableFor(selHero, arena, c, r);
