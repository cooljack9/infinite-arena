...
export default function PreBattle() {
  ...
  // vX 队长系统：本局已选队长 uid（跟随镜头目标）；铁人模式队长阵亡后清空需重选
  const leaderUid = useGame((s) => s.run?.leaderUid ?? null);
  const setLeaderUid = useGame((s) => s.setLeaderUid);
  ...
  const confirm = () => {
    ...
    // ★ 这里是**权威提交点**（preview:false）：玩家布阵已定稿（上一行 setFormation 已同步写入 store），
    //   服务端据此结算 + 落库 + 推进层数，返回 battleId 供 parity ack。
    //   休整间隙的 IntermissionHub.prefetchNext 只发 preview（不结算、不锁层），故此处不会吃到陈旧布阵。
    if (isRemoteMode() && run.runId) {
      ...
      void getBackend().startBattle({
        ...
        if (r.ok) {
          useGame.getState().setPrefetchBattle({ data: r.data, key, mode: battleCtx.mode, runLayer: run.layer, formation: useGame.getState().formation });
        } else {
  ...
  return (
    <div className="app">
      <div className="panel col">
        ...
        {/* vX 队长系统：战前选择队长（跟随镜头目标）。首战强制选；铁人模式队长阵亡后强制重选。未选禁开战。 */}
        <div className="subtitle" style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 6 }} id="tut-prebattle-leader">
          选择队长：
          {leaderUid === null && (
            <span className="chip" style={{ color: '#ff6a6a', borderColor: '#ff6a6a' }}>
              {run.mode === 'ironman' ? '⚠ 队长已阵亡，必须选择新队长' : '⚠ 请选择本队队长（跟随镜头目标）'}
            </span>
          )}
        </div>
        <div className="row">
          {run.team.map((h) => (
            <button
              key={h.uid}
              className={leaderUid === h.uid ? 'primary' : ''}
              onClick={() => setLeaderUid(h.uid)}
              style={{ flex: '1 1 30%', minWidth: 110, textAlign: 'left', lineHeight: 1.4 }}
            >
              <div className="hero-name">{h.personalName ?? h.name}{leaderUid === h.uid ? ' ★' : ''}</div>
              <div className="tag">{h.name}{h.star ? ` · ${h.star}★` : ''}</div>
            </button>
          ))}
        </div>
        ...
        <div id="tut-prebattle-skip" className="col" style={{ marginTop: 8, gap: 6 }}>
          <div className="row between">
            ...
            <div className="row" style={{ gap: 8 }}>
              ...
              <button id="tut-prebattle-start" className="primary" disabled={!picked || leaderUid === null || countdown > 0} onClick={confirm}>
          ...
          {picked && leaderUid === null && countdown === 0 && (
            <div className="muted" style={{ fontSize: 12, color: '#ffb37a', textAlign: 'right' }}>
              ⚠ 请先在上方「选择队长」中点选一名队长，才能开战
            </div>
          )}
        </div>
      </div>
