...
import { SubClass, type BattleStatRow, type Vec2 } from '@arena/core/types';
import ArenaCanvas, { type KillEvent } from '../render/ArenaCanvas';
...
/** 布阵等价判定：uid 集合一致且每个 uid 的 (x,y) 一致 → 可安全复用预热回放 */
function formationsEqual(a: Record<string, Vec2>, b: Record<string, Vec2>): boolean {
...
function BattleScreenInner({ run }: { run: NonNullable<ReturnType<typeof useGame.getState>['run']> }) {
  ...
  // vX 击杀播报：战场 killfeed 滚动条，由 ArenaCanvas 的 onKill 事件驱动（纯展示，零 core 改动）
  // 新条目从底部进入，旧条目逐条上移、并随高度(索引)逐渐淡化+缩小 → 形成「往上滚 + 渐隐缩小」的播报条
  const [kills, setKills] = useState<Array<{ id: number; ts: number } & KillEvent>>([]);
  const killSeq = useRef(0);
  const handleKill = useCallback((k: KillEvent) => {
    setKills((prev) => [{ id: ++killSeq.current, ts: Date.now(), ...k }, ...prev].slice(0, 7));
  }, []);
  // 新一场战斗（sim 变化 = 换层/换局）清空击杀播报
  useEffect(() => { setKills([]); }, [sim]);
  // 6s 后自动移除最旧条目，避免无限堆积（索引淡化已让旧条目近乎不可见）
  useEffect(() => {
    if (kills.length === 0) return;
    const tm = setTimeout(() => setKills((prev) => prev.filter((x) => Date.now() - x.ts < 6000)), 1500);
    return () => clearTimeout(tm);
  }, [kills]);
  // vX 击杀播报样式（自包含）：底部锚定、向上滚动、按索引(--i)淡化(--o)/缩小(--s)
  const killfeedCss = `
.killfeed{position:absolute;left:50%;top:30%;height:0;width:0;pointer-events:none;z-index:5;}
.killfeed-item{position:absolute;left:0;bottom:0;transform-origin:center bottom;
  font:bold 12px ui-monospace,monospace;padding:3px 8px;border-radius:6px;
  background:rgba(8,10,18,0.74);border:1px solid rgba(255,255,255,0.08);
  color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:82vw;
  transform:translateX(-50%) translateY(calc(var(--i) * -26px)) scale(var(--s));
  transition:transform .32s cubic-bezier(.22,.61,.36,1), opacity .32s ease;
  animation:kfIn .25s ease-out;}
.killfeed-enemy{color:#ff8a8a;border-left:3px solid #ff4a4a;}
.killfeed-ally{color:#9fc0ff;border-left:3px solid #4aa3ff;}
@keyframes kfIn{from{opacity:0;transform:translateX(-50%) translateY(10px) scale(.9);}to{opacity:var(--o);}}
`;
  ...
  const ackParity = useCallback(async (
    ...
    if (!battleId) return; // preview 回放未落库（battleId=null），无 battle 行可 ack；提交点会另行 ack
    try {
  ...
  useEffect(() => {
    ...
    // 每层战斗重置幂等 key：BattleScreen 在自动爬塔续层时**不重挂**（App 用同一位置渲染，
    // 无 key prop），沿用上一层的 key 会让后端按旧 key 命中缓存 / 语义串层。
    battleKeyRef.current = null;
    // v1.8.1 优先采用预热好的权威回放，消除「正在进入战场」黑屏。
    // ★ 只消费**权威（已提交）**预热：battleId 非空 = preview:false 已结算落库；
    //   preview 预热（battleId=null，未结算）绝不能当权威回放用，否则层数不推进。
    // ★ 且布阵必须与当前一致：玩家在布阵页交换位置后，陈旧布阵的预热一律作废、走提交重算。
    const pf = useGame.getState().prefetchBattle;
    const ctxMode = useGame.getState().battleCtx.mode;
    const curFormation = useGame.getState().formation;
    if (
      pf && pf.data.battleId && pf.mode === ctxMode && pf.runLayer === run.layer
      && formationsEqual(pf.formation, curFormation)
    ) {
      useGame.getState().setPrefetchBattle(null);
      setBattleRemote(battleRemoteOf(pf.data)); // 单次上屏：观感=结算，且不触发 sim 重建
      battleKeyRef.current = pf.key; // 复用提交时的幂等 key，settle 兜底重发时后端不重复结算
      void ackParity(pf.data.battleId, pf.data.replay, { runId: run.runId, idempotencyKey: pf.key, coreVersion: CORE_VERSION });
      return;
    }
    // 无可用权威预热（含：只有 preview 预热 / 布阵已改 / 上个提交失败）→ 挂载即提交
    useGame.getState().setPrefetchBattle(null);
  ...
  const settle = useCallback((result: 'win' | 'lose') => {
        ...
        if (!remote) {
          ...
          try {
            const r = await getBackend().startBattle({
              ...
              formation: useGame.getState().formation,
              clientTs: Date.now(),
              battleOpts: mods, // skip5 兜底提交也要带多层参数，否则会按普通战斗结算（奖励/层跳都错）
            });
    ...
    // 战报 stats：复用 onEnd 在 combat 结束瞬间抓的快照（含阵亡者完整数据），避免 2s 横幅后漏阵亡者
    const localBattleStats = statsSnapRef.current ?? sim.getBattleStats();
    ...
    if (result === 'win') {
      ...
      if (ctxNow.mode === 'skip5') {
        ...
        recordBattleEval(localBattleStats, 'win', snap.layer + 5, snap.layer + 6, cap2);
        return;
      }
      ...
      if (ctxNow.mode === 'climb' && useGame.getState().climbSession) {
        ...
        if (stop) {
          ...
          setBattleCtx({ mode: 'normal' });
          recordBattleEval(localBattleStats, 'win', cur?.layer ?? snap.layer, (cur?.layer ?? snap.layer) + 1, cap2);
          return;
        }
      ...
      recordBattleEval(localBattleStats, 'win', snap.layer, next2, cap2);
    } else {
      const ctxNow = useGame.getState().battleCtx;
      ...
      if (ctxNow.mode === 'climb' && useGame.getState().climbSession) {
        ...
        recordBattleEval(localBattleStats, 'lose', cur?.layer ?? snap.layer, cur?.layer ?? snap.layer, cap2);
        return;
      }
      ...
      if (ctxNow.mode === 'skip5') {
        ...
        recordBattleEval(localBattleStats, 'lose', effLayer, effLayer, cap2);
        return;
      }
      ...
      const failures = (snap.failures ?? 0) + 1;
      setFailures(failures);
      recordBattleEval(localBattleStats, 'lose', snap.layer, snap.layer, cap2);
    }
  }, [sim, recordBattleEval, commitGrowth, collectLoot, removeDeadAllies, addScore, setFailures,
      setLayer, setBattleCtx, climbReward, effLayer, mods]);

  // 战报 stats 快照：combat 结束瞬间（onEnd 首次触发）抓一次 sim.getBattleStats()，
  // 此时阵亡单位尸体仍在 units 中（deadAt 距今 < CORPSE_TTL），含其完整「造成/承受/治疗」数据。
  // 2s 横幅后再抓会因尸体已按 CORPSE_TTL 移除而漏掉阵亡者 → 战报三项数据全 0 / 阵亡敌消失。
  const statsSnapRef = useRef<BattleStatRow[] | null>(null);

  const onEnd = useCallback((result: 'win' | 'lose') => {
    // v3.2 战斗结束横幅挂 2s（胜利/失败中央大字），再进入结算/战报——给玩家情绪落点，也给后端结算留缓冲
    if (banner) return;
    // 关键修复：立即快照（仅本地；远程走云端 outcome.stats，由服务端在 over 时算，不丢阵亡者）
    if (!isRemoteMode() && sim) {
      try { statsSnapRef.current = sim.getBattleStats(); } catch { /* sim 已释放则退回实时抓取 */ }
    }
  ...
  }, [banner, settle, recordBattleEval, sim]);
  ...
  return (
    <div className="app battle-stage">
      ...
      {isRemoteMode() && !battleRemote?.replay ? (
        ...
        <div className="battle-canvas-area" style={{ position: 'relative' }}>
          <ArenaCanvas sim={sim} running={!sim.over && !paused} speed={battleSpeed} onEnd={onEnd} onKill={handleKill} />
          {/* vX 击杀播报：战场击杀/阵亡滚动条，底部锚定、向上滚动并逐条淡化缩小 */}
          <style>{killfeedCss}</style>
          <div className="killfeed">
            {kills.map((k, i) => {
              const s = Math.max(0.5, 1 - i * 0.14); // 越旧(索引越大)越小
              const o = Math.max(0.1, 1 - i * 0.26); // 越旧越淡
              const st = { opacity: o, ['--i']: i, ['--s']: s, ['--o']: o } as CSSProperties;
              return (
                <div
                  key={k.id}
                  className={'killfeed-item' + (k.side === 'ally' ? ' killfeed-ally' : ' killfeed-enemy')}
                  style={st}
                >
                  {k.side === 'enemy'
                    ? (k.killer ? `${k.killer} 击杀 ${k.victim}` : `击杀 ${k.victim}`)
                    : `${k.victim} 阵亡`}
                </div>
              );
            })}
