...
// v1.8.3 卡死修复：锻造/融合页同样全量渲染背包装备（无上限），安卓 WebView 会爆主线程。
// 共用分页：每页固定件数，列表变化自动回第一页。
const FORGE_PAGE = 24;
function usePaged<T>(items: T[]) {
  const [page, setPage] = useState(0);
  const total = Math.max(1, Math.ceil(items.length / FORGE_PAGE));
  const safe = Math.min(page, total - 1);
  useEffect(() => { setPage(0); }, [items]);
  return {
    items: items.slice(safe * FORGE_PAGE, (safe + 1) * FORGE_PAGE),
    total,
    page: safe,
    setPage,
    pager: total > 1 ? (
      <div className="row" style={{ gap: 6, marginTop: 4, alignItems: 'center' }}>
        <button className="tag" style={{ cursor: 'pointer', padding: '2px 8px' }} disabled={safe === 0} onClick={() => setPage(safe - 1)}>‹ 上一页</button>
        <span className="muted" style={{ fontSize: 10 }}>{safe + 1}/{total}</span>
        <button className="tag" style={{ cursor: 'pointer', padding: '2px 8px' }} disabled={safe >= total - 1} onClick={() => setPage(safe + 1)}>下一页 ›</button>
      </div>
    ) : null,
  };
}
...
export default function ForgeTab() {
  ...
  const showTip = (t: string) => { setTip(t); window.setTimeout(() => setTip(''), 1250); }; // 提示驻留统一 1.25s
  ...
  // v1.8.3 分页（背包无上限，防安卓渲染卡死）
  const tfP = usePaged(tf.filtered);
  const mfP = usePaged(mf.filtered);
  const ffP = usePaged(ff.filtered);
  ...
  const doReforge = () => {
    ...
    setReforging(true);
    setFxBusyWaves('重铸中…', '已完成重铸');
    reforgeItem(forgeTarget);
    setForgeTarget(null);
    setTimeout(() => { setReforging(false); setFxBusy(null); }, 1250); // 响应上限 1.25s
  };
  ...
  const doFuse = () => {
    ...
    setFxBusyWaves('正在融合装备…', '已完成融合装备');               // 两段波：遮罩 + 锁全局交互
    fuse(fuseSel[0], fuseSel[1]);
    setFuseSel([]);
    setTimeout(() => { setFusing(false); setFxBusy(null); }, 1250); // 响应上限 1.25s
  };
  ...
  return (
    <div className="col" style={{ marginTop: 4 }}>
      ...
      {/* v1.3 进度条：一眼看出本层合成额度用了几成 */}
      <div className="progress" style={{ marginTop: 6 }} role="progressbar" aria-valuemin={0} aria-valuemax={2} aria-valuenow={Math.min(2, fusedThisLayer)} aria-label={`本层已合成 ${Math.min(2, fusedThisLayer)}/2`}>
        <div className="progress__bar" style={{ width: `${Math.min(100, (Math.min(2, fusedThisLayer) / 2) * 100)}%` }} />
      </div>
