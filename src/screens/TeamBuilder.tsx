...
import { SUBCLASS_INFO, ROLE_CN, rangeLabelOf } from '@arena/core/content/classes';
...
export default function TeamBuilder() {
  ...
  const depart = () => {
    ...
    startRun(team);
    // vX 出征台词（弹字动画）开始即预载布阵 chunk，与 startRun 并行；覆盖层遮住 Suspense 兜底，避免任何「加载中…」透出
    void import('./PreBattle');
          ...
          {HEROES.map((h) => {
            ...
            return (
              ...
              >
                ...
                <div className="hero-cat">{rangeLabelOf(h.subclass)} · {ROLE_CN[info.role]}</div>
              </div>
            );
          })}
        </div>
