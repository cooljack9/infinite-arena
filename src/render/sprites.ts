...
const HI = 0.26;   // 受光棱提亮（vX 质感加强：斜切更明显）
const LO = -0.42;  // 背光面压暗（vX 质感加强：AO 更深）

/**
 * 逐像素定光照档位：假定光源在左上方（像素画通用约定，也是玩家直觉）。
 * 判据只看四邻是否实心——O(1)、与模板无关，天然沿剪影内缘生成一圈内斜面。
 */
function toneAt(tpl: string[], r: number, c: number, mat: string): number {
  const h = tpl.length;
  ...
  // ① 边缘斜面浮雕：左上方受光、右下方背光（保留原 2-档行为，不破坏剪影）
  let tone = 0;
  if (!up && !left) tone += HI;
  else if (!down || !right) tone += LO;
  // ② 体积 form-shadow：自顶部提亮、向底部平滑压暗，让躯干/头读成圆柱而非平板。
  //    归一化到模板高度（与像素尺寸无关），纯算术 O(1)，只在 raster 时算一次 → 运行时零成本。
  //    vX 质感加强：顶/底幅度拉大，圆柱体积更明显、剪影更有"重量"。
  const v = (r / h - 0.5) * 2;          // 顶 -1 → 底 +1
  tone += v < 0 ? v * 0.08 : v * 0.18;  // 顶提亮至多 +0.08、底压暗至多 -0.18
  // ③ 材质分层高光（烘焙，零运行时成本）：金属(m/M)更高对比 + 受光边 glint + 中段竖向镜面带，布料/皮肤保持哑光
  if (mat === 'm' || mat === 'M') {
    tone *= 1.4;
    if (!up && !left) tone += 0.16;     // 受光边高光点，读作反光硬金属
    tone += 0.10 * (1 - Math.min(1, Math.abs(v))); // 中段竖向镜面带：抛光金属的高光弧，强化"金属质感"
  }
  return tone;
}
...
function rasterize(tpl: string[], o: RasterOpts): HTMLCanvasElement {
  ...
  for (let r = 0; r < h; r++) {
    for (let c = 0; c < w; c++) {
      ...
      if (ch !== 'O' && ch !== 'g' && ch !== 'e') color = shade(color, toneAt(tpl, r, c, ch));
      g.fillStyle = color;
...
export interface MountDrawOpts {
  t?: number;        // 当前时间，驱动步态
  tb?: number;       // vX 抗弹跳：速率钳制后的身体时钟（起伏/前倾/侧摆用它，避免加速期被震成高频）
...
const MOUNT_GAIT: Record<MountKind, { freq: number; amp: number; bounce: number }> = {
  elephant: { freq: 5.5, amp: 0.022, bounce: 0.003 },  // 沉稳如鼓（降弹）
  leopard:  { freq: 8.5, amp: 0.032, bounce: 0.018 },  // 轻快（降频降弹，少"弹跳"）
  tiger:    { freq: 6.5, amp: 0.026, bounce: 0.010 },  // 肌肉起伏（降弹）
  redhare:  { freq: 10,  amp: 0.036, bounce: 0.014 },  // 疾驰生风（降频降弹）
  ox:       { freq: 6.5, amp: 0.020, bounce: 0.006 },  // 沉重碾地（降弹）
};

/** 坐骑贴图相对骑手 renderPx 的宽度倍率。战象最壮、玄豹最修长——
 *  vX 把跨度从原 1.42–1.55（≈9%）拉大到 1.32–1.62（≈23%），让体型本身也成为识别特征 */
const MOUNT_SCALE: Record<MountKind, number> = {
  elephant: 1.62, leopard: 1.32, tiger: 1.45, redhare: 1.50, ox: 1.55,
};

/** 骑手相对坐骑背脊的上抬量（占骑手 renderPx 的比例）。
 *  vX：随 MOUNT_SCALE 等比缩放（lift ∝ scale），保证放大/缩小坐骑时骑手 seating 关系不变 */
export const MOUNT_RIDER_LIFT: Record<MountKind, number> = {
  elephant: 0.42, leopard: 0.22, tiger: 0.28, redhare: 0.34, ox: 0.34,
};
...
export function drawMount(
  ...
  const t = opts.t ?? 0;
  const tb = opts.tb ?? t; // vX 抗弹跳：起伏/前倾/侧摆用钳制后的身体时钟
  ...
  if (opts.casting) {
    ...
    lean = 0.08 + Math.sin(tb * g.freq * 0.5) * 0.02;
    swayX = Math.sin(tb * g.freq) * w * 0.012;
  } else {
    // 静止 idle 专属微动作（用钳制时钟 tb，加速期不抖）：让坐骑有生命感而非僵立
    switch (kind) {
      case 'redhare':  swayX = Math.sin(tb * 2.4) * w * 0.010; break; // 鬃毛轻摆
      case 'elephant': lean  = Math.sin(tb * 1.1) * 0.030; break;    // 长鼻缓摆（躯干微俯仰）
      case 'ox':       lean  = Math.sin(tb * 1.6) * 0.020; break;    // 铁角轻晃
      case 'leopard':  lean  = Math.sin(tb * 2.0) * 0.025; break;    // 头部微颤
      case 'tiger':    lean  = Math.sin(tb * 1.4) * 0.020; break;    // 尾根微动
    }
  }
  const gait = opts.moving
    ? Math.sin(tb * g.freq) * h * g.amp + Math.abs(Math.sin(tb * g.freq * 0.5)) * h * g.bounce
    : Math.sin(tb * 2.0) * h * 0.012;
