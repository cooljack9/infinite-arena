...
// vX 身份铭牌：友方英雄用成长原型色（神化金/极致蓝/无界粉）做姓名铭牌底色，零 core/parity 风险。
// persona 只依赖 @arena/core（rng/types/scaling），不反向依赖 render，无循环依赖。
import { archetypeOf, ARCHETYPE_INFO } from '../game/persona';
...
// vX 位移平滑：临界阻尼弹簧跟随（渲染层私有状态，零 sim 改动）。20Hz 逻辑步进在 tick 边界
// 速度不连续（起步-匀速-急停）→ 纯线性插值仍显机械。这里用弹簧把渲染位向 sim 位低通跟随，
// 速度连续、带轻微惯性，单位移动更顺滑「有重量」；按真实帧 dt 积分，帧率无关。
const renderPos = new Map<string, { x: number; y: number; vx: number; vy: number }>();
let renderDt = 1 / 60;
const MOVE_SPRING = 17; // 自然角频率 ≈ 17 rad/s（时间常数 ~59ms），平滑且跟手不拖影
// vX 受击边沿检测：记录每单位上一帧 flash 值，0→>0 即「本帧刚被命中」，触发轻微震屏（友方/Boss 才有，避免群怪互殴震屏噪声）
const flashPrev = new Map<string, number>();
...
export function drawFrame(
  ...
  lastWall = wall;
  renderDt = dtw; // vX 位移弹簧：喂入真实帧 dt（限幅后），保证帧率无关
  ...
  if (sim.time < fx.lastSimT - 0.05) {
    ...
    renderPos.clear(); // vX 位移弹簧：重开清空渲染位，避免旧场坐标漂移残留
    flashPrev.clear(); // vX 受击边沿：重开清空，防跨场误触发
  }
  ...
  // vX 受击反馈：友方 / Boss 单位被命中（flash 0→>0 边沿）触发轻微震屏 + 极短顿帧，
  // 让「挨打」有触感（此前仅重击/技能有打击感，普通攻击静默 → 打击感弱）。群怪互殴不触发，避免满屏噪声。
  if (wall >= fx.stopUntil) {
    for (const u of sim.units) {
      if (!u.alive) { flashPrev.set(u.id, 0); continue; }
      const f = u.flash ?? 0;
      const prev = flashPrev.get(u.id) ?? 0;
      if (f > 0 && prev <= 0 && (u.side === 'ally' || u.isBoss)) {
        triggerFx(wall, 0.045, u.isBoss ? 4 : 2.5, 0.12, null, 0, 0);
      }
      flashPrev.set(u.id, f);
    }
  }
  ...
  // vX P1 聚焦目标：被多数存活友方锁定的敌方单位 = 团队当前集火目标（纯渲染派生，零 sim 改动、确定性零影响）
  let focusId: string | undefined;
  {
    const counts = new Map<string, number>();
    let bestN = 0;
    for (const a of sim.units) {
      if (a.alive && a.side === 'ally' && a.targetId) {
        const n = (counts.get(a.targetId) ?? 0) + 1;
        counts.set(a.targetId, n);
        if (n > bestN) bestN = n;
      }
    }
    if (bestN > 0) {
      ...
      for (const [fid, n] of counts) {
        if (n !== bestN) continue;
        const fu = sim.units.find((x) => x.id === fid && x.alive);
        if (fu) { const f = fu.hp / fu.maxHp; if (f < bestHp) { bestHp = f; focusId = fid; } }
      }
    }
  }
  curIds.clear();
  for (const u of sim.units) { curIds.add(u.id); drawUnit(ctx, u, skin, t, alpha, sim.terrainCraters, bodyTime, focusId); }
  for (const id of birthStart.keys()) if (!curIds.has(id)) birthStart.delete(id);
  for (const id of renderPos.keys()) if (!curIds.has(id)) renderPos.delete(id); // vX 离场单位清渲染位，防长局内存累积
  for (const id of flashPrev.keys()) if (!curIds.has(id)) flashPrev.delete(id); // vX 离场单位清受击边沿状态
...
function drawUnit(
  ...
  // vX 位移平滑：先取 20Hz 线性插值目标位，再用临界阻尼弹簧低通跟随，
  // 速度连续、带轻微惯性 → 单位移动更顺滑「有重量」（消除线性插值起步-急停的机械感）。
  // 纯渲染层私有状态，零 sim 改动、确定性/回放 parity 零影响。
  const tgX = alpha > 0 && u.prevX !== undefined ? u.prevX + (u.x - u.prevX) * alpha : u.x;
  const tgY = alpha > 0 && u.prevY !== undefined ? u.prevY + (u.y - u.prevY) * alpha : u.y;
  let rp = renderPos.get(u.id);
  if (!rp) { rp = { x: tgX, y: tgY, vx: 0, vy: 0 }; renderPos.set(u.id, rp); }
  else {
    const dt = renderDt;
    const w = MOVE_SPRING;
    // 半隐式欧拉 + 临界阻尼（2w 阻尼项）：零超调、帧率无关（dt 已限幅 ≤0.05，w·dt<1 稳定）
    const ax1 = w * w * (tgX - rp.x) - 2 * w * rp.vx;
    const ay1 = w * w * (tgY - rp.y) - 2 * w * rp.vy;
    rp.vx += ax1 * dt; rp.vy += ay1 * dt;
    rp.x += rp.vx * dt; rp.y += rp.vy * dt;
  }
  const ax = rp.x, ay = rp.y;
  ...
  if (u.alive && castT >= 0 && castT < 0.28) {
    ...
    // vX P1 放大招技能名浮标：施法窗口内头顶浮出职业大招名（"在放啥"一目了然，纯渲染）
    if (u.alive) {
      const castT2 = u.castAnimAt !== undefined ? t - u.castAnimAt : 999;
      if (castT2 >= 0 && castT2 < 0.28) {
        const sub = SUBCLASS_INFO[u.subclass];
        const skName = sub ? sub.cn : u.subclass;
        const sy2 = cy - px * 1.20;
        const a2 = 0.35 + 0.65 * Math.sin(Math.min(1, castT2 / 0.28) * Math.PI);
        ctx.save();
        ctx.globalAlpha = a2;
        ctx.textAlign = 'center';
        ...
        ctx.strokeText(`${skName}·大招`, cx, sy2);
        ctx.fillStyle = sub ? sub.color : '#ffffff';
        ctx.fillText(`${skName}·大招`, cx, sy2);
        ctx.restore();
      }
    }
  ...
  if (mo.hud) {
    ...
    // vX P1 聚焦高亮：团队集火目标画金色四角括号（"打谁"视觉锚点，纯渲染派生，零 sim 改动）
    if (focusId && u.id === focusId) {
      const fr = rr * 1.5;
      const bx0 = cx - fr, by0 = cy - px * 0.95, bw2 = fr * 2, bh2 = px * 1.35;
      const arm = Math.min(fr * 0.6, px * 0.4);
      const pulse = 0.6 + 0.4 * Math.sin(t * 10 + idPhase(u.id));
      ctx.save();
      ctx.strokeStyle = `rgba(255,210,74,${0.7 + 0.3 * pulse})`;
      ctx.lineWidth = 2;
      // 左上
      ctx.beginPath(); ctx.moveTo(bx0, by0 + arm); ctx.lineTo(bx0, by0); ctx.lineTo(bx0 + arm, by0); ctx.stroke();
      // 右上
      ctx.beginPath(); ctx.moveTo(bx0 + bw2 - arm, by0); ctx.lineTo(bx0 + bw2, by0); ctx.lineTo(bx0 + bw2, by0 + arm); ctx.stroke();
      // 左下
      ctx.beginPath(); ctx.moveTo(bx0, by0 + bh2 - arm); ctx.lineTo(bx0, by0 + bh2); ctx.lineTo(bx0 + arm, by0 + bh2); ctx.stroke();
      // 右下
      ctx.beginPath(); ctx.moveTo(bx0 + bw2 - arm, by0 + bh2); ctx.lineTo(bx0 + bw2, by0 + bh2); ctx.lineTo(bx0 + bw2, by0 + bh2 - arm); ctx.stroke();
      ctx.restore();
    }
  ...
  // vX P0 身份铭牌：战场最大可读性缺口是"认不出谁是谁"（用户反馈"啥也看不清"）。
  // 友方英雄：姓名 + 原型色徽章（神化金/极致蓝/无界粉，来自 persona.archetypeOf）；
  // 敌方：仅 Boss / 反堆一人(focusRole) 显示姓名（精英怪需被识别，杂兵不刷屏避免噪声）。
  // 纯渲染，零 core/parity 风险；每单位仅 1 次 fillText，混战开销可忽略。
  {
    const showName = u.side === 'ally' ? !!u.heroUid : !!(u.isBoss || u.focusRole);
    if (showName) {
      const ny = by - 9;
      let badge = u.side === 'ally' ? '#4aa3ff' : '#ff5a5a';
      if (u.side === 'ally' && u.heroUid) {
        badge = ARCHETYPE_INFO[archetypeOf({ uid: u.heroUid, id: u.heroUid })].color;
      }
      ctx.save();
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ...
      ctx.strokeText(u.name, cx, ny);
      ctx.fillStyle = badge;
      ctx.fillText(u.name, cx, ny);
      // 友方英雄再补一道原型色小横杠（身份锚点）
      if (u.side === 'ally' && u.heroUid) {
        ctx.fillStyle = badge;
        ctx.fillRect(cx - 8, ny + 5, 16, 1.5);
      }
      ctx.restore();
    }
  }
