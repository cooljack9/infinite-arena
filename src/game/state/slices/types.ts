...
import type { GrowthChoiceRecord } from '../../persona';
...
export type PrefetchBattle = {
  ...
  mode: BattleCtx['mode'];
  key: string;
  /** 该预热回放所用的布阵；BattleScreen 仅当与当前布阵一致才复用，否则重新提交 */
...
export interface MetaSlice {
  ...
  // vX 战场缩放（camera zoom，账号级，持久化）：'auto' 自适应 / 1.0 全战场 / 1.25 温和拉近 / 1.5 明显拉近
  cameraZoom: 'auto' | number;
  // vX 镜头模式：全图（看整张战场）/ 跟随队长（镜头居中于队长单位）。默认全图。
  ...
  // vX 战场缩放（camera zoom）：仅影响渲染合成期，零 sim/parity 影响
  setCameraZoom: (v: 'auto' | number) => void;
  // vX 镜头模式（全图 / 跟随队长）：仅影响渲染合成期，零 sim/parity 影响
  setCameraMode: (v: 'full' | 'follow') => void;
...
export interface RunSlice {
  ...
  growthLog: GrowthChoiceRecord[]; // 人格化：成长自我选择履历（传记「意志」来源），本地字段不进云端快照
  /**
  ...
  /** vX 队长系统：设定本局队长 uid（战前中枢选择；铁人模式队长阵亡后强制重选） */
  setLeaderUid: (uid: string) => void;
  ...
  // 人格化：成长自我选择写回（按 uid 写入 growthBonus + 追加 growthLog 履历）
  chooseGrowth: (entries: { uid: string; growth: HeroGrowth; log: GrowthChoiceRecord }[]) => void;
