...
import { rollRecruitPool, HERO_BY_ID } from '@arena/core/content/heroes';
import {
  discountOf, recruitCostOf, REFRESH_COST, FUSE_PER_LAYER, TEAM_CAP,
  BREAKTHROUGH_MAIN_CHANCE, hashStr, nextHeroUid, equipCapOf,
} from './helpers';
...
function equipLocal(get: () => GameState, set: (p: Partial<GameState>) => void, uid: string, eqId: string): boolean {
  ...
  if (cur.length >= equipCapOf(hero)) return false; // 槽位已满（成长者仅 3）
...
export const createEconomySlice: StateCreator<GameState, [], [], EconomySlice> = (set, get) => ({
  ...
  if (isRemoteMode()) {
    ...
    for (const item of pool) {
      ...
      let bestFree = 0;
      for (const h of targets) {
        const free = equipCapOf(h) - equipped[h.uid].length;
  ...
  if (isRemoteMode()) {
    ...
    // v1.7 招募池刷新时即个体化（同角色不同副本基础值/体型/姓名各异），价格据此浮动
    const taken = run.team.map((h) => h.personalName ?? h.name);
    set({
      gold: get().gold - REFRESH_COST,
      recruitPool: rollRecruitPool(mulberry32(seed), run.team).map((h, i) =>
        variateHero(h, (seed ^ (i * 0x9e3779b1)) >>> 0, taken)),
      refreshCount: rc,
    });
  },
  ...
  if (isRemoteMode()) {
    ...
    if (!hero) return;
    // v1.7 招募池已个体化，直接采用池中英雄（不再二次个体化）；
    //       价格按实际基础值相对预设的偏离浮动（贵≠一定强，偏高无用属性也会抬价）
    const preset = HERO_BY_ID[hero.id]?.basePrimary;
    const cost = recruitCostOf(run.layer, hero.basePrimary, preset);
    if (get().gold < cost) return;
    if (run.team.length >= TEAM_CAP) return; // 满编不可再招募
    const dupIndex = run.team.filter((h) => h.id === heroId).length + 1;
    const copy = { ...hero, uid: nextHeroUid(), star: 1, dupIndex };
    set({
      gold: get().gold - cost,
