...
import {
  ...
  loadCameraZoom, saveCameraZoom, loadCameraMode, saveCameraMode,
  MAX_PRESETS,
} from './helpers';
...
export const createMetaSlice: StateCreator<GameState, [], [], MetaSlice> = (set, get) => ({
  ...
  // vX 战场缩放（camera zoom，账号级偏好，跨局/跨会话保持）
  cameraZoom: loadCameraZoom(),
  // vX 镜头模式：全图 / 跟随队长（账号级偏好，跨局/跨会话保持）
  ...
  // vX 战场缩放（camera zoom）：渲染层以 ref 模式读取，改完立刻生效（无需重开战斗）
  setCameraZoom: (v) => {
    saveCameraZoom(v);
    set({ cameraZoom: v });
  },
