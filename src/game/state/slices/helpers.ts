...
// vX 战场缩放（camera zoom，账号级，持久化）：'auto' 自适应（默认）/ 1.0 全战场 / 1.25 温和拉近 / 1.5 明显拉近。
// 'auto' 按屏幕宽度 + 设备像素比自动换算实际放大倍数：窄屏物理像素不足 → 1.0 不裁切（看清全场），
// 宽屏 / 高分屏 → 自动拉近（封顶 1.35）让单位更大更清晰。仅影响渲染合成期「整张战场围绕中心放大裁切」，零 sim/parity 影响。
const ZOOM_KEY = 'ia_zoom';
export const CAMERA_ZOOM_DEFAULT = 'auto' as const;
export const loadCameraZoom = (): 'auto' | number => {
  try {
    const raw = localStorage.getItem(ZOOM_KEY);
    if (raw === 'auto' || raw === null) return CAMERA_ZOOM_DEFAULT;
    const v = parseFloat(raw);
    return v === 1 || v === 1.25 || v === 1.5 ? v : CAMERA_ZOOM_DEFAULT;
  } catch { return CAMERA_ZOOM_DEFAULT; }
};
export const saveCameraZoom = (v: 'auto' | number) => {
  try { localStorage.setItem(ZOOM_KEY, String(v)); } catch { /* ignore */ }
};

// vX 镜头模式（camera mode，账号级，持久化）：'full' 全图 / 'follow' 跟随队长。默认全图。
const CAM_MODE_KEY = 'ia_cammode';
export const CAMERA_MODE_DEFAULT = 'full' as const;
export const loadCameraMode = (): 'full' | 'follow' => {
  try {
    const v = localStorage.getItem(CAM_MODE_KEY);
    return v === 'follow' ? 'follow' : CAMERA_MODE_DEFAULT;
  } catch { return CAMERA_MODE_DEFAULT; }
};
export const saveCameraMode = (v: 'full' | 'follow') => {
  try { localStorage.setItem(CAM_MODE_KEY, v); } catch { /* ignore */ }
};
