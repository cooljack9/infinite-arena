...
export const createRunSlice: StateCreator<GameState, [], [], RunSlice> = (set, get) => ({
  ...
  startRun: (team, mode = get().selectedMode) => {
    ...
    set({
      run: {
        ...
        leaderUid: null, // vX 队长系统：开局未选，由 PreBattle 选定（铁人队长阵亡后清空重选）
      },
      screen: 'pre',
  ...
  // vX 队长系统：战前中枢（PreBattle）选定队长 uid，写回 run.leaderUid（跟随镜头目标）。
  // 队长阵亡（铁人 permadeath）后由 removeDeadAllies 清空，强制下一战重选。
  setLeaderUid: (uid) => {
    const run = get().run;
    if (!run) return;
    set({ run: { ...run, leaderUid: uid } });
  },
  ...
  removeDeadAllies: (uids) => {
    ...
    // vX 队长系统：若阵亡者含队长，清空 leaderUid → 下一战 PreBattle 强制重选
    const leaderDead = run.leaderUid ? uidSet.has(run.leaderUid) : false;
    set({
      run: {
        ...run,
        team: run.team.filter((h) => !uidSet.has(h.uid)),
        leaderUid: leaderDead ? null : run.leaderUid,
      },
      equipped,
  ...
  chooseGrowth: (entries) => {
    if (isRemoteMode()) return;
    const run = get().run;
    if (!run) return;
    const byUid = new Map(entries.map((e) => [e.uid, e]));
    const team = run.team.map((h) => {
      const e = byUid.get(h.uid);
      return e ? { ...h, growthBonus: addGrowth(h.growthBonus, e.growth) } : h;
    });
    set({ run: { ...run, team }, growthLog: [...get().growthLog, ...entries.map((e) => e.log)] });
  },
