...
export interface StartBattleReq extends CommandEnvelope {
  ...
  /**
   * preview=true：只算回放、不结算、不落库、不推进 run（休整间隙的纯预热用，battleId 返回 null）。
   * 缺省 / false：真正提交——结算 + 落库 + 推进层数，battleId 非空。
   * 用途：避免「预热抢跑结算」把层数提前推进、并用陈旧/空 formation 锁死该层
   *       （导致战前交换位置不生效）。
   * 提交点：PreBattle.confirm（玩家布阵定稿那一刻）为权威提交；
   *        BattleScreen.fetchBattle 为兜底提交（预热缺失 / 布阵已改 / 提交失败时）。
   */
  preview?: boolean;
}
...
export interface BattleResultDTO {
  /** 提交（preview=false）时非空；preview=true 的纯预热回放为 null（无落库战斗行） */
  battleId: string | null;
  /** preview=true 标记：本响应是纯预热回放，未结算、未落库 */
  preview?: boolean;
