...
import { TRAIT_CFG, STAGE2_CFG, TRAITS } from '../content/traits';
...
export class BattleSim {
  ...
  private ultBurst(
    ...
    // 内环比爆点小且晚 0.08s：读作「能量回吸」，给爆炸一个收势
  ...
  private moveToward(u: Unit, target: Unit, dt: number) {
    ...
    const sp = (2.0 + dampMs * 0.02) * glide * Math.max(0.2, slow) * terrainSlow;
  ...
  private applyDamage(
    ...
    dmgMult *= this.traitOutMult(attacker, target); // 攻击方特性：穿透减伤封顶
  ...
  private dealSkill(u: Unit, target: Unit, amount: number, type: Unit['damageType'], crit = false) {
