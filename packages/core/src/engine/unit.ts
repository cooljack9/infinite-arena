...
import { applyTraitStatic, TRAIT_CFG } from '../content/traits';
...
export function makeAlly(
  ...
  // vX 成长率 = 基础值 / 初始设定值，钳制 [25%,175%]；该副本的全部属性成长按成长率缩放。
  // 基础值漂移 ±75% → 成长率各异，使基础值本身成为成长潜力判定（确定性不变：纯乘区，同 seed 同结果）。
  // 实际成长还叠加击杀随机（见 battle.ts creditKillGrowth），故最终成长仍受随机击杀情况影响。
  const tBase = hero.templateBase ?? hero.basePrimary;
  const growRate = (k: keyof PrimaryAttrs) =>
    Math.max(0.25, Math.min(1.75, (hero.basePrimary[k] || 0) / (tBase[k] || hero.basePrimary[k] || 1)));
  const growth: PrimaryAttrs = {
  ...
  // vX 英雄数值微调（需求逐项百分比乘区）：仅在 makeAlly 落地，不污染成长/装备/体型链；
  // 因此「物攻-30%」按派生 pDmg 整体乘 0.7，而非只削 str（避免牵连暴伤/减伤等派生项错配）。
  if (hero.mods) {
  ...
  // vX 成长者：体型 -30%（离散体型无法连续缩放，用 sizeScale 缩放命中半径/渲染）
  if (hero.traitId === 'grower') u.sizeScale = 1 - TRAIT_CFG.growerBodyPct;
  // vX 归来者：防御恒为 0（物理/魔法减伤清零；render 若有护甲条也按 0 显示）
  if (hero.traitId === 'returner') { u.derived.pResist = 0; u.derived.mResist = 0; }
  return u;
}
