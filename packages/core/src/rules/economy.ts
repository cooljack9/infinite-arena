...
// v1.3 招募价：随层数缓增（基础价）
// v1.7 叠加「基础值偏离预设」倍率：传入英雄实际 basePrimary 与模板预设 preset 时，
//   四项平均高于预设 ≥10% → ×1.10；≥30% → ×2.00；低于预设 ≤−20% → ×0.75；其余 ×1.00。
//   不传 base/preset（升星/突破等）则退回纯层数价。
export const recruitCostOf = (layer: number, base?: PrimaryAttrs, preset?: PrimaryAttrs): number => {
  const baseCost = 60 + 20 * layer;
  if (!base || !preset) return baseCost;
  const keys: (keyof PrimaryAttrs)[] = ['con', 'str', 'agi', 'int'];
  let sumDev = 0;
  for (const k of keys) {
    const p = preset[k] || 1;
    sumDev += (base[k] - p) / p;
  ...
  let mult = 1;
  if (avgDev >= 0.30) mult = 2.0;       // 基础值平均高 30% 以上 → 价格 +100%
  else if (avgDev >= 0.10) mult = 1.1;  // 基础值平均高 10% 以上 → 价格 +10%
  else if (avgDev <= -0.20) mult = 0.75;// 基础值平均低 20% 以上 → 价格 −25%
