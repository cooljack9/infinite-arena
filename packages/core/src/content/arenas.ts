...
/** 在指定行找一个 '.' 放出生点 */
const placeSpawn = (tiles: string[], rng: RNG, ch: 'S' | 'E', rowBand: number[]) => {
...
// 强度全部锁在 ±15% 内（上表最大 12%），天气是「风味 + 微小变量」，不是第三个养成系统。
export const WEATHER_BY_THEME: Record<MapTheme, WeatherDef> = {
  sandstone: { kind: 'sandstone', cn: '风沙',     icon: '🌪', moveSpeedAdd: 10 },   // 移速 +10%
  frost:     { kind: 'frost',     cn: '霜寒',     icon: '❄', atkSpeedAdd: -12 },   // 攻击间隔 +12%（atkSpeed −12）
  magma:     { kind: 'magma',     cn: '熔火',     icon: '🔥', dmgMul: 1.12 },        // 伤害 +12%
  void:      { kind: 'void',      cn: '虚空侵蚀', icon: '🌌', critAdd: 8 },          // 暴击 +8%
  verdant:   { kind: 'verdant',   cn: '丰茂',     icon: '🌿', regenPct: 0.012 },     // 每秒回复 1.2% 最大生命
  sanctum:   { kind: 'sanctum',   cn: '圣光',     icon: '✨', dmgTakenMul: 0.88 },   // 受到伤害 −12%
};
