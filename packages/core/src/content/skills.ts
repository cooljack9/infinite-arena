...
// v1.5 全局放大系数（美术 §7.3⑤）：技能特效半径 = castRange × TILE × VFX_SCALE × sizeMul。
// v2.3 1.4→1.5、v2.8 1.5→1.85 都是「更明显」方向；v2.9.x 按需求②「特效与实际范围对齐 +
// 降光污染」反转：1.85×是光污染 + 特效不贴合判定的根因（视觉半径 ≈ 实际 2.4×，玩家学到错误
// 的安全距离）。回落到 1.0 —— 此时视觉半径 = castRange × sizeMul，sizeMul(0.9~1.5) 是
// 有意的「签名强调」而非任意膨胀。若实测又嫌弱，调 sizeMul 而非回头拉 VFX_SCALE。
export const VFX_SCALE = 1.0;
...
export const SKILL_VFX: Record<SkillStyle, SkillVfx> = {
  ...
  van_ram:           { color: '#ffcf3d', sizeMul: 1.40, motion: 'charge_wedge' }, // v2.9.x 面包车撞击
};
