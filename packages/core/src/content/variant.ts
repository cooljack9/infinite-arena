...
import { rollTrait, TRAITS } from './traits';

const round1 = (v: number) => Math.round(v * 10) / 10;

// vX 基础属性波动幅度扩大到 ±75%：0.25~1.75。
// 配合「成长率 = 基础值 / 初始设定值」机制（见 unit.ts makeAlly）：基础值漂移直接决定成长潜力，
// 同一模板不同副本的成长率因此落在 [25%,175%]，避免所有角色成长比例基本一致。
// 仍由 mulberry32 种子驱动，确定性不变；定价据此按相对预设的偏离浮动（见 rules/economy.ts）。
const VAR_LO = 0.25;
const VAR_HI = 1.75;
...
export function variateHero(base: HeroDef, seed: number, takenNames: Iterable<string> = []): HeroDef {
  ...
  // ── vX 角色特性：从全池随机分配（用户需求「所有角色都随机产生」）──
  // HEROES 模板已不再绑定固定 traitId（见 heroes.ts），故 base.traitId 恒为 undefined，
  // 此处必走 rollTrait(rng) 从全池抽一个；已在 base.personality 之后取 rng()，确定性不变。
  // 已生成/已存档副本（base.traitId 已填）仍走 ?? 保留原特性，满足旧存档兼容。
  const traitId = base.traitId ?? rollTrait(rng);

  // vX 模板原始基础值：用于「成长率 = 基础值 / 初始设定值」。
  // 仅在首次个体化的模板上记录（base.templateBase 未定义时取 base.basePrimary 预设值）；
  // 已个体化副本保留其原模板值（?? 回退），保证 v3.x 旧存档 + 同 seed 的回放兼容。
  const templateBase = base.templateBase ?? base.basePrimary;

  return {
    ...base, basePrimary, templateBase, bodyType, gender, personalName, personality, traitId,
