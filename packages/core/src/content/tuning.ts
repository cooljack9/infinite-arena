...
import { overrideEnemyFocus, type EnemyFocusCfg } from '../engine/coherence';

export interface Tuning {
  /** 无尽缩放曲线参数（见 scaling.ts 的 ScaleCfg） */
  scaling?: Partial<ScaleCfg>;
  /** 敌方针对最强（见 coherence.ts 的 EnemyFocusCfg）：反"堆一人"轻量方案 */
  enemyFocus?: Partial<EnemyFocusCfg>;
}
...
export function applyTuning(t: Tuning): void {
  ...
  if (t.enemyFocus) overrideEnemyFocus(t.enemyFocus);
