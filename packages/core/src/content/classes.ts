...
// v4.0 阵容定位（用户需求：编队卡第二排要一眼看出 近战/远程 + 角色定位）
// 4 桶：肉盾 / 输出 / 控制 / 奶妈（与 category 不同——mage 下细分出 控制/奶妈/输出）
export type HeroRole = 'tank' | 'dps' | 'control' | 'healer';
export const ROLE_CN: Record<HeroRole, string> = {
  tank: '肉盾',
  dps: '输出',
  control: '控制',
  healer: '奶妈',
};
// 射程阈值：attackRange>=4 视为远程（炮/射/术/召唤/医），否则近战（坦/战/剑）
export const rangeLabelOf = (sub: SubClass) => (SUBCLASS_INFO[sub].attackRange >= 4 ? '远程' : '近战');
...
export const SUBCLASS_INFO: Record<SubClass, SubClassInfo> = {
  ...
  hexblade:   { category: 'warrior', role: 'dps',     name: 'hexblade',   cn: '无名剑客',  damageType: 'hybrid',  attackRange: 3.0, color: '#cfe3ff', color2: '#eaf3ff', defaultBody: 'light' },
  gunner:     { category: 'archer',  role: 'dps',     name: 'gunner',     cn: '神机炮兵',  damageType: 'physical', attackRange: 5.0, color: '#ff9a3c', color2: '#ffd9a8', defaultBody: 'heavy' },
  ...
  controller: { category: 'mage',    role: 'control', name: 'controller', cn: '太极术师',  damageType: 'magic',    attackRange: 6.0, color: '#7fe0d8', color2: '#d6fffb', defaultBody: 'medium' },
  summoner:   { category: 'mage',    role: 'dps',     name: 'summoner',  cn: '化生术师',  damageType: 'magic',    attackRange: 5.0, color: '#c79a5a', color2: '#e8d3ad', defaultBody: 'medium' },
  healer:     { category: 'mage',    role: 'healer',  name: 'healer',    cn: '回春医官',  damageType: 'magic',    attackRange: 5.0, color: '#4fd982', color2: '#d6ffe6', defaultBody: 'petite' },
};
