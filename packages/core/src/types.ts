...
export interface BattleStatRow {
  ...
  isDead: boolean;       // 本场阵亡标记（仅展示 / MVP 记账不变）；从 u.alive 反推，确定性安全
}
...
export interface RunState {
  ...
  // vX 队长系统：本局被指定为「队长」的友方英雄 uid（跟随镜头目标；铁人模式队长阵亡后强制重选）。null = 尚未选择。
...
export interface AudioCue {
  ...
  /** vX 击杀播报：death 类 cue 携带阵亡者姓名与击杀者姓名，供战场 killfeed 渲染（纯展示，不影响 sim 确定性） */
  victim?: string;
  /** vX 击杀播报：击杀者姓名（敌方阵亡时为我方英雄；我方阵亡时为空） */
  killer?: string;
}
