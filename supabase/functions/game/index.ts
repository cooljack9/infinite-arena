...
async function handleBattle(p: Record<string, unknown>, ports: ReturnType<typeof denoPorts>, runOf: (id: string) => Promise<RunRow | null>) {
  ...
  // ★ preview：纯预热，只算回放、不结算、不落库、不推进 run。
  // 用途：休整间隙的预热（IntermissionHub）在玩家敲定布阵前就发，用 preview 避免「抢跑结算」
  // 把层数提前推进、并用陈旧/空 formation 锁死该层（→ 战前交换位置不生效）。
  // 权威提交点是 PreBattle.confirm（玩家布阵定稿那一刻）；BattleScreen.fetchBattle 为兜底提交。
  if (p.preview === true) {
    ...
    return ok({
      ...
      replay: {
        ...
        layer: (p.battleOpts as BattleOptsDTO | undefined)?.effLayer ?? layer,
        mode: (row.snapshot as unknown as RunSnapshot).mode,
        arena: b.arena,
        allies: b.allies,
        enemies: b.enemies,
        buildings: b.buildings,
        buildingScale: b.buildingScale,
        checksum: b.checksum,
        vanEncounter: b.vanEncounter,
      },
      outcome: {
        result: b.result,
        ...
        stats: b.stats,
        mvpUid: b.mvpUid ?? null,
        mvpStat: b.mvpStat ?? null,
        mvpAdd: b.mvpAdd ?? 0,
        killGains: b.killGains,
        deadAllyUids: b.deadAllyUids,
      },
      snapshot: row.snapshot as unknown as RunSnapshot,
    });
  }

  // ★ 同层幂等去重：本层已结算过 → 直接回放已有 battle 行，避免重复结算 / 重复建行 / 乐观锁互踩。
  // 进战 fetchBattle、PreBattle.confirm、settle 兜底可能各发一次 startBattle（幂等 key 各不相同），
  // 不加这层，并发时同一层会被 runBattle+applySettlement 多次重算，行与状态双双污染。
  // （休整间隙的预热走 preview 分支，不落库、不参与去重。）
